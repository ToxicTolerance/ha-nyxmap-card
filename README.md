...
| `cluster_radius` | number | largest configured entity `size` (48 if none set) | Pixel radius (per zoom level) within which entities collapse into one bubble. Every marker is a circle exactly `size` px across, so this default approximates "cluster once markers would actually touch." Lower it for tighter grouping, or raise it for looser — this is still a MapLibre-native limitation: clustering buckets by a fixed radius per *whole* zoom level, not by continuously tracking each entity's actual rendered position, so it can still look premature partway through a zoom level even with a well-tuned value. |
| `cluster_max_zoom` | number | `14` | Zoom level at and above which clustering stops entirely, regardless of how close entities are. |
