...
describe("IconButtonControl", () => {
  it("renders a maplibregl-ctrl-group button with an inline svg icon/label and wires clicks to onClick", () => {
    const onClick = vi.fn();
    const control = new IconButtonControl({ iconPath: "M1,1H2V2H1Z", label: "Toggle grouping", onClick });
    ...
    expect(button.querySelector("svg path")?.getAttribute("d")).toBe("M1,1H2V2H1Z");
  ...
  it("does not set aria-pressed when isPressed is unset", () => {
    const control = new IconButtonControl({ iconPath: "M1,1H2V2H1Z", label: "Reset focus", onClick: vi.fn() });
    const el = control.onAdd() as HTMLElement;
  ...
  it("onRemove() detaches the container from the DOM", () => {
    const control = new IconButtonControl({ iconPath: "M1,1H2V2H1Z", label: "Toggle grouping", onClick: vi.fn() });
    const el = control.onAdd() as HTMLElement;
